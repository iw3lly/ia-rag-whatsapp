...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  try {
    ...
    if (settingsError || !settings?.openrouter_api_key) {
      throw new Error('Configurações de IA incompletas ou ausentes.');
    }
    ...
    const { data: contextChunks, error: contextError } = await supabase.rpc(
    ...
    ${system_prompt}
    Sua resposta deve ser baseada **EXCLUSIVAMENTE** nos documentos de contexto fornecidos.
    ...
    const aiResponse =
      ...
      'Não consegui gerar uma resposta coerente com o contexto.';
    ...
    return res.status(500).json({ error: 'Erro interno ao processar a IA: ' + e.message });
  }
};
